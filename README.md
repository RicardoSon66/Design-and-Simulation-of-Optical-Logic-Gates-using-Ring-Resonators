...
커플링이 이루어져 링 도파로로 들어간 빛이 회전을 하면서 사진상으로 보았을때 Output D로는 빛이 흘러가지 않고 C로만 흘러가는 모습을 볼 수 있습니다.  
다음은 B에 입력을 가해보겠습니다.  
...
선형 세계에서의 빛이 서로 충돌을 하였으므로 사진과 같이 빛이 보강, 상쇄 현상이 연속적으로 이루어짐을 볼 수 있으나 여기서 위상이 반전되지는 않습니다.  
...
마지막으로 A, B동시에 입력을 가하면 두 수치가 각 입력마다 측정 하였을때 나온 값과 거의 근사한 값이고 0과 멀기 때문에 C, D의 출력이 1이다.  
라는걸 증명하는 수치입니다.  
...
Meep FDTD simulation 에서는 이 heater에 포함되어있는 material를 지원을 안합니다...  
