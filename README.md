...
이번 프로젝트는 간단한것 같으면서도 생각보다 굉장히 어려운 프로젝트 였습니다.  
단순하게 TR로 Gate를 구현하는거 처럼 하면 되겠지~ 이런 생각을 가졌지만 막상 해볼려고 하니 정말 오래 걸린 것 같습니다.  
AND-Gate를 구현할때가 가장 오래걸렸는데 구현 난이도 보다는 "내가 어떻게 설계를 할 것이냐?" 에 대한 막힘이 있어서 그런것 같았습니다.
그리고 깨달은건 설계라는건 설계자의 생각이 100% 반영이 되는것 설계자마다 같은 결과여도 완전히 다른 설계를 가질 수 있다는 것  
단순하게 구현하고 "아 잘나온다" 하고 구현 했습니다 라고 보여주는 것 보다 왜 이렇게 되는가에 대해 설명을 꼭 해야한다는게 필요 하기에  
이론적인 부분으로 설명을 하기 위해서 조사하고 공부 하는것도 오래 걸린 것 같습니다.  
다음 프로젝트는 ALU를 만들어 볼까 합니다.  
ALU도 끝내면 뭘 할거냐 라는 생각이 들수도있는데 일단 1차적으로는 매우 단순한 CPU 설계 및 구현이 목표입니다.  

저의 프로젝트 결과물을 보았을때 이미 인지를 하신 분들이 있을거라고 생각합니다.  
...
또한 프로젝트는 휘발성으로 남겨둘 생각은 없고 장기적으로 계속 업데이트를 해나갈 생각입니다.
