...
기본적으로 NOT Gate를 제외하면 논리 연산에 두가지 입력이 필요하므로 프로젝트에 채용한 소자는 상단과 하단에 버스 도파로가 배치된  
Dual-port Ring Resonator 입니다.  
...
이렇기에 heater 도입을 하지 말아야하나.. 라고 생각을 하였지만 방법은 있었습니다.  
...
바로 NOT-Gate의 입출력을 바꾸는건 heater를 작동시키는 **"전기신호"** 입니다.
heater는 phase shift를 위해 실리콘 도파로에 열을 가하여 굴절률을 높게해 위상이 느려지게 하는 역할입니다.

**3.입력 테스트**

(사진)  

이번에는 Heater가 True 입니다.
Heater가 True면 굴절률이 증가하므로 링 내부로 들어가는 빛이 거의 0에 수렴합니다.  
이를 뒷받침할 증거는 flux를 측정한 것 입니다.  
